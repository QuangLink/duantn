...
import { Box, Heading, Text, Divider, Image, Center, Flex, Button, Checkbox, VStack,useDisclosure,useToast } from '@chakra-ui/react';
...
function MyOrder() {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:9000/orders/user/${userID}`);
      ...
      ([orderCode, productList]) => (
        <React.Fragment key={orderCode}>
          ...
          {productList.map((product) => (
            ...
            <Box mt="5" display="flex" justifyContent="space-evenly" borderWidth="1px" borderRadius="md" bg={'white'}>
              
                <Box key={product.id} p={4} borderRadius="md" mb={4}>
                    <img src={product.prodImg} width="200px" />
                </Box>
                <Box width="60%">
                    <Text mt="2" height="50px" fontFamily="serif" color="#424245" noOfLines={2} fontSize="20px" fontWeight="700">
                    ...
                    <Text fontWeight="600" fontSize="18px" ml="1" color="black">
                        Purchase Time: {new Date(product.orderDate).toLocaleString()}
                    </Text>
                    <Text fontWeight="600" fontSize="18px" ml="1" color="black">
                        Delivery Time: {new Date(product.orderDate).toLocaleString()}
                    </Text>
                </Box>
                <Box mt="50">
                    <Text class="badge bg-danger" fontWeight="600" fontSize="18px" ml="1" color="black" >
            ...
            <Box>
                ...
                <Box mt="50" >
                    <Button type="submit" colorScheme="blue" marginRight={20}>Mua lại</Button>
                    <Icon as={BsFillTrashFill} w={6} h={6} color='black' margin="auto" />
                </Box>
            </Box>
        </Box>
          ))}
  ...
  const renderAddressData = () => {
    ...
    return addressData.map((address, index) => (
      ...
      <Image src="https://example.com/avatar.jpg" alt="" borderRadius="full" boxSize="150px" justifyContent="center" />
      <hr />
      <VStack spacing="10px" marginTop="20px">

          <Text fontSize="xl">{address.firstname} {address.lastname}</Text>
    ...
    return (
        ...
        <Box display="flex" justifyContent="center">
            ...
            <Box>
                <Box textAlign="center" padding="50px" margin="10px 0px" borderWidth="1px" width="80%" borderRadius="md">
                {renderAddressData()}
                    
                </Box>
            </Box>
            <Box width="70%">
                <Heading as="h2" size="lg" mb={4}>My Order</Heading>
                {renderProducts()}
                <br />
            </Box>
        </Box>
